...
Please be aware that numerous steps in our regression case of random forests align with those from our classification example. Consequently, we will provide a more concise overview of the regression case when in steps that align in both examples.
...
Next, we define a workflow (`rf_auto_wf`) to combine our regression model (`rf_auto`) and our data preparation recipe (`recipe_auto`).
...
The next step is to create our tuning grid and tune the model to find the most optimal configuration of our hyper parameters.
...
The model seems to have better performance with a smaller minimum node size, the number of trees doesn't appear to affect the performance and the performance of the model plateaus after 4 predictors. The best performing model had 8 randomly sampled predictors, 300 trees and a minimum of 10 data points in a node. It had a mean RMSE of 2.906 (we will explain what RMSE means in the "Accuracy Measures" section).

Lastly, we can extract the best configuration of hyper parameters with `select_best(tune_auto)`, finalize our workflow with `finalize_workflow(rf_auto_wf, best_rf_auto)`, and fit our model with `fit(final_auto_model, auto_train)`.
...
An RMSE of 2.002811 indicates that, on average, the model's predictions are 2.002811 units away from their observed values. The model's fit based off the RMSE depends on the specifics of the data and the required precision of the problem, but with a range of 31.1 (in `mpg`), an RMSE of 2.002811 is considered relatively low. Therefore, our model has decent predictive accuracy.
